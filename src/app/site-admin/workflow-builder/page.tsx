...
const visitorTypes = ['Interviewee', 'Vendor', 'Contractor', 'VIP', 'Govt Official', 'Delivery', 'Client / Business Partner', 'Maintenance / Service Technician', 'Press / Media', 'Intern / Trainee', 'Auditor / Inspector', 'Consultant'];
...
function WorkflowBuilderContent() {
  ...
  const workflowName = searchParams.get('workflow') || 'Pre-Registered Visitor';
      ...
      <div className="flex flex-col" style={{ height: 'calc(100vh - 96px)', overflow: 'hidden' }}>
        ...
        <div className="flex items-center justify-between px-5 py-3 bg-white border-b border-gray-200 flex-shrink-0 gap-4">
          ...
          <div className="flex items-center gap-3 min-w-0">
            <Link href="/site-admin/workflows">
              <button
            ...
            <button className="flex items-center gap-2 font-bold text-gray-900 hover:text-[#6C47FF] transition-colors min-w-0 group">
              <span className="truncate max-w-48">{workflowName}</span>
              <Settings2 className="w-3.5 h-3.5 text-gray-400 group-hover:text-[#6C47FF] flex-shrink-0" />
            </button>
            <span className="flex-shrink-0 text-[10px] text-text-muted font-medium">{siteName}</span>
                  ...
                  {expandedCategories.includes(cat.id) && (
                    <div>
                      {cat.fields.map(field => (
                        <div
                          key={field.id}
                          className="flex items-center gap-2.5 px-2 py-2 rounded-xl hover:bg-gray-50 cursor-grab active:cursor-grabbing transition-colors mb-1"
                        >
                          <div
                            className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${field.colorClass} ${field.bgClass}`}
                          >
                            {field.icon}
                          </div>
                          <div className="flex-1 min-w-0">
...
export default function SiteAdminWorkflowBuilderPage() {
  return (
    <Suspense fallback={<div className="flex items-center justify-center h-screen text-sm text-gray-400">Loading workflow builder…</div>}>
